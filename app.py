...
def edit_quote_date(id):
    ...
    try:
        ...
        flash(f'Date for quote #{id} updated to {new_dt.strftime("%d/%m/%Y %H:%M")}.', 'success')
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error updating date for quote {id}: {str(e)}")
        flash(f'Error updating date for quote #{id}. Please try again.', 'error')
...
def edit_quote_text(id):
    ...
    try:
        ...
        flash(f'Quote #{id} text updated successfully.', 'success')
    except Exception as e:
        db.session.rollback()
        logging.error(f"Error updating text for quote {id}: {str(e)}")
        flash(f'Error updating quote #{id}. Please try again.', 'error')
